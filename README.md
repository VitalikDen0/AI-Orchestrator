...
- **Генерация видео**: Функция реализована, но находится в стадии тестирования (к сожалению мой пк слишком слаб, плюс пока не понимаю как правильно такое сделать, ни в AUTOMATIC1111, ни в ComfyUI не получилось нормально реализовать. Fun Fact, ранее генерация была через AUTOMATIC1111, потом я понял что слишком неоптимизированно, перешел на ComfyUI и так и не смог разобраться, вечная Ernno 22)
- **Распознавание видео**: Экспериментальная функция, не полностью протестирована (не было нужны :] )


### Текущие проблемы

- **LM Studio API**: После версии 0.3.23 возникают проблемы с автозагрузкой моделей и показом активных моделей
- **Требуется ручная загрузка**: Модели нужно предварительно загружать в LM Studio (возможно я разберусь с проблемой, модель moondream2-llamafile нормально запускается, в ближайшем обновлении, а может и уже будет исправлено)
