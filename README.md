...
~~Рефакторинг, запуск около 30-40 секунд, а это долго, я не хочу ждать. Плюс ОЧЕНЬ тяжелый промт, буду как нибудь думать о его умной разбивке, где нейросеть получает информацию о инструментах и тд, ТОЛЬКО если надо, но я не хочу делать огромный словарь слов, по которым эти промты для работы будут передаваться, я хочу чтобы нейросеть сама понимала когда ей нужна дополнительная информация, а это уже надо думать. Я буду играться с промтом и другими (если есть такое) методами для реализации фикса этой большой проблемы. Промт в 8.5к токенов (около 20к символов по-моему) это действительно уже ни в какие рамки не влазит. (ну то что модель может работать с 262к токенов не означает что ее можно запустить настолько, я использую KV кэш с Q8 и могу максимум запустить 150к, ниже Q8 слишком сильная дергадация, тем более, чем больше контекста тем ЗНАЧИТЕЛЬНО падает скорость. У меня базово 90-110 токенов в секунду, при 150к+- контекста скорость +-25 токенов в секунду, это СУЩЕСТВЕННОЕ падение скорости. Да, на таких значениях промт в 7к токенов не выглядит чем-то страшным, но этот промт уже делает скорость генерации около 70 токенов в секунду. Скриншоты примеров ниже. В общем я не хочу класть огромный болт на оптимизацию со словами "Да хрен с ней, когда нибудь потом". Если я не разберусь с этим сейчас, в будущем я не исправлю эту мной наделанную хрень никогда)~~ Upd: Короче мне было лень пихать все в основной файл, поэтому я просто создал папку promts, где разместил все промты, разбитые по логике. Пока не пробовал работу, но должно работать (будет кое какой фикс еще)
<details>
  <summary>Показать скриншоты</summary>
  <p>
    <img src="https://github.com/user-attachments/assets/3da5f86c-4891-4800-b276-703b402d107e" alt="screenshot 1" width="49%">
    <img src="https://github.com/user-attachments/assets/f1f08481-5456-414b-84ea-c029f2bc5fa8" alt="screenshot 2" width="49%">
  </p>
</details>

LoRA на фикс пальцев, короче реализация LoRA для изображений
