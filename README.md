...
- **Linux**: Постепенное обновление кода под Linux
- **.bat скрипт**: Для упрощенной установки и начала работы
- **Управление через Telegram**: Возможность отправлять команды/запросы боту вместо консоли или web
...
- Вынес все что нужно в config.py. Со временем разобъю файл на части, в планах до десяти
- Проделано ОЧЕНЬ много работы, все перенес в разные файлы, нет времени на тестирование, как появится, протестирую. Но основной фунционал точно работает.
...
- **Возможны проблемы с первым запуском**: Есть вероятность, что при первом запуске может вылезти ошибка, но попробуйте еще раз запустить. Так как я не уверен что она будет у всех, но я столкунлся с этой проблемой. Проблема холодного старта библиотек. Это можно решить, но на данный момент это слишком неважно, чтобы тратить на это время.
