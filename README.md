...
- Параллельно есть мысли об интеграции с BrowserOS, мне не нужно будет создавать **Управление браузером**, я буду использовать уже готовое. Возможно я передумаю, но создавать нечто такое с нуля будет для меня неподъемной задачей.
- А ну и надо сделать так, чтобы было удобно устанавливать. Реализую через .bat или/и .sh1
- Очень вероятно что я все же перейду на llama-cpp-python или подобные библиотеки. Так как код достаточно сильно связан с LM Studio будет тяжеловато, но это будет лучше, так как не нужно будет устанавливать дополнительные приложения.
- WEB перейдет на Node.js... Возможно, с помощью него смогу сделать гораздо лучше все
...
- **Web интерфейс**: Web интерфейс достаточно сильно отстает от реализации через консоль, это будет позже поправлено. Попробую сделать его по лучше, хотя я не очень хочу, так как буду пробовать переходить к нативному GUI и попробую отказаться от Web, но там как пойдет (ну он нормально выполняет свой фунционал, но это будто содрали с сайта 2000 годов)
